...
Constant NKEY__TX     = "N = next";
Constant PKEY__TX     = "P = previous";
Constant QKEY1__TX    = "         Q = exit";
Constant QKEY2__TX    = "Q = previous menu";
Constant RKEY__TX     = "RETURN = select";
...
Array TitleStringForWidth buffer 128;
...
Class Option
  with
    emblazon [ bar_height page pages temp;
      ...
      ! Calculate centering for title and then print it
      @output_stream 3 TitleStringForWidth;
      print (name) self;
      if (pages ~= 1) print " [", page, "/", pages, "]";
      @output_stream -3;
      temp = (screen_width - TitleStringForWidth-->0)/2;
      @set_cursor 1 temp;
      print (name) self;
      if (pages ~= 1) print " [", page, "/", pages, "]";
    ],
