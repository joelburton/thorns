...
def classify_lines(lines):
    ...
    while i < len(lines):
        ...
        if stripped.startswith('> '):
            ...
            cmd = stripped[2:]
            if cmd.startswith('* '):
                # Section header comment — skip the following [Comment recorded] line
                result.append(('section_header', cmd[2:].strip()))
                i += 1
                # Skip blank lines then [Comment recorded] if present
                while i < len(lines) and not lines[i].strip():
                    i += 1
                if i < len(lines) and lines[i].strip() == '[Comment recorded]':
                    i += 1
            else:
...
def to_html(classified):
    ...
    for kind, text in classified:
        ...
        if kind == 'command':
            ...
            parts.append(f'<h2 class="section-header">{t}</h2>')
        elif kind == 'room_header':
            parts.append(f'<h3 class="room-header">{t}</h3>')
        elif kind == 'paragraph':
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  h2.section-header {
    ...
    border-bottom: 2px solid rgba(156,124,58,0.5);
    text-align: center;
  }
